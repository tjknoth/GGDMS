...
namespace BucketMultiselect_timers{
  using namespace std;
...
#define CUDA_CALL(x) do { if((x) != cudaSuccess) {      \
  ...
  */
  ...
  inline void timing_switch(int option, int ind, float* timers){
    if(option == 0) {
  ...
  inline void timing(int option, int ind, float* timers){
    #ifdef TIMING_ON 
      timing_switch(option, ind, timers);
    #endif
  }
  ...
  __global__ void copyElements_tree (T* d_vector, int length, uint* elementToBucket
    ...
    if(idx < length) {
      for(int i=idx; i<length; i+=offset) {
        ...
        if (active) {
          newArray[atomicDec(d_bucketCount + blockIdx.x * numTotalBuckets + temp_active, length)-1] = d_vector[i];
        }  // end if (active)
...
//    thrust::sort(randoms_ptr, randoms_ptr + sizeOfSample);
  ...
  */
  ...
  T bucketMultiSelect (T* d_vector, int length, uint * kVals, int numKs, T * output, int blocks
              , int threads, int numBuckets, int numPivots, float * timers) {    
...
timing(1,6,timers);
  ...
  T bucketMultiselectWrapper (T * d_vector, int length, uint * kVals_ori, int numKs
                              , T * outputs, int blocks, int threads, float * timers) { 
    ...
    bucketMultiSelect<T> (d_vector, length, kVals, numKs, outputs, blocks, threads, numBuckets, 17, timers);
