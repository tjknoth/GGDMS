...
namespace BucketMultiselect_mgpu{
  using namespace std;
...
/*
        ...
        for(int j=1; j < numPivots - 1; j*=2){
            PivotIndex = (PivotIndex << 1) | (num >= sharedPivotTree[Shift + PivotIndex]);
            Shift = (Shift << 1 ) | 1;
        }
...
/*
        ...
        for(int j = 1; j < numPivots - 1; j*=2) {
          ...
          compare = (num >= sharedPivots[midPivotIndex]);
          minPivotIndex =  compare ? midPivotIndex : minPivotIndex;
          maxPivotIndex = compare ? maxPivotIndex : midPivotIndex;
        }
...
slopes[numPivots - 1]=0;
  ...
  */
  template <typename T>
  T bucketMultiSelect_mgpu (T* d_vector, int length, uint * kVals, int numKs, T * output, int blocks
...
/*
    ...
    CUDA_CALL(cudaMalloc(&d_pivots, numPivots * sizeof(T)));
    CUDA_CALL(cudaMalloc(&d_pivottree, (numPivots - 2) * sizeof(T)));
*/
...
//    thrust::sort(newInput_ptr, newInput_ptr + newInputLength);
    ...
    mgpuDeviceSort<T>(newInput, newInputLength);
   ...
   */
  template <typename T>
  T bucketMultiselectWrapper_mgpu (T * d_vector, int length, uint * kVals_ori, int numKs
    ...
    */
    ...
    bucketMultiSelect_mgpu (d_vector, length, kVals, numKs, outputs, blocks, threads, numBuckets, 17);
