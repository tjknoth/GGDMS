...
namespace BucketMultiselect_cub{
  using namespace std;
...
//    thrust::sort(randoms_ptr, randoms_ptr + sizeOfSample);
  ...
  */
  template <typename T>
  T bucketMultiSelect_cub (T* d_vector, int length, uint * kVals, int numKs, T * output, int blocks
    ...
    if ( (numUniqueBuckets < 1) ) { 
      ...
      copyElements_tree<T><<<numBlocks, threadsPerBlock, 
        numUnique_extended * sizeof(uint)>>>(d_vector, length, d_elementToBucket, 
                                             ...
                                             d_bucketCount, numBuckets);
    } // end if - else numUniqueBuckets for copy
...
timing(1,6);
   ...
   */
  template <typename T>
  T bucketMultiselectWrapper_cub (T * d_vector, int length, uint * kVals_ori, int numKs
    ...
    */
    ...
    bucketMultiSelect_cub<T> (d_vector, length, kVals, numKs, outputs, blocks, threads, numBuckets, 17);
