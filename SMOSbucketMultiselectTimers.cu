...
namespace CompareMultiselect {
  ...
  */
template<typename T>
void bmsSubroutineTimer(uint size, uint* kVals, uint numKs, uint numTests
    ...
    for(type=0; type<2; type++){
      ...
      case 0:
        ...
        snprintf(fileName, 128, 
               "bmsTimers: %s %s k-dist:%s 2^%d to 2^%d (%d:%d:%d) %d-tests on %s at %s", 
