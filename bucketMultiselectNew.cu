...
#define CUDA_CALL(x) do { if((x) != cudaSuccess) {      \
  ...
  */
  ...
  // ****** mgpu merges sort typically faster for doubles *****
  // **********************************************************
  template <typename T>
  ...
  */
  ...
  /* Function to rebucket values after reducing the problem
   * 
   * Note: Declare slopes and endpoints vectors. Length is now
   *       Length of reduced vector.
   *       Can we use elementToBucket or do we need to declare
   *       a new array?
   *       Why subtract 1 if first parameter < second parameter?
   */
  template <typename T>
  __global__ void rebucketValues (T* d_vector, int length, uint* elementToBucket
                                  , uint* d_bucketCount, int buckets, double* slopes
                                  , T* endpoints, int numBlocks, int offset) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (idx < buckets) {
      ...
      for (int i = 0; i < idx; i++) {
        cumulativeCount += d_bucketCount[i * numBlocks];
        endpoints[i] = cumulativeCount;
      }
      thrust::device_ptr<T>dev_ptr(d_vector);
      thrust::pair<thrust::device_ptr<T>, thrust::device_ptr<T> > result = 
        thrust::minmax_element(dev_ptr + offset, dev_ptr + offset + d_bucketCount[idx * numBlocks]);
      min = *result.first;
      max = *result.second;
      slopes[idx] = (max - min) / d_bucketCount[idx * numBlocks];
    }

    syncthreads();

    for (int i = idx; i < length; i += offset) {
      elementToBucket[i] = (int) atomicDec((d_vector[i] - endpoints[i]) * slopes[i], buckets) - 1;
    }
  }


  /* Function to check that no marked buckets are empty
   *
   * Note: Check that int/uints are all correct
   *       Make sure orderStats exists
   */
  template <typename T>
  __global__ void checkMarkedBuckets (T* d_vector, int length, uint* elementToBucket
                                      , int* markedBuckets, T* orderStats, T* endpoints
                                      , double* slopes, int buckets, uint* d_bucketCount, int numBlocks) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (idx < buckets) {
      if (slopes[idx] * (d_bucketCount[idx * numBlocks]) < MIN_SLOPE) {
        orderStats[i] = d_vector[idx * numBlocks]
        markedBuckets[i] = 0;
      }
    }
  }
...
timing(0,2);
    ...
    CUDA_CALL(cudaMalloc(&d_slopes, numPivots * sizeof(double)));
...
//     return 1;
  ...
  // MAKE SURE length, d_elementToBucket IS UPDATED

  double* d_slopes;
  T* d_endpoints; 
  CUDA_CALL(cudaMalloc (&d_slopes, numUniqueBuckets));
  CUDA_CALL(cudaMalloc (&d_endpoints, numUniqueBuckets));

  rebucketValues<<<numBlocks, threadsPerBlock>>>(d_vector, length, d_elementToBucket, d_bucketCount 
                                                 , numUniqueBuckets, d_slopes, d_endpoints, numBlocks, offset);
