...
% vector length; focus on small lengths to show CPU performance.
...
for v=1:1
    ...
    pname=['CPUTimeGrowthLengthVecDist' vec{v} '.pdf'];
    for t=1:3
        ...
        % alternate formulation - throw out the min and max for plotting
        data(:,3) = ( data(:,3).*data(:,15) - data(:,4) - data(:,5) ) ./ ( data(:,15)-2 );
        data(:,7) = ( data(:,7).*data(:,15) - data(:,8) - data(:,9) ) ./ ( data(:,15)-2 );
        data(:,11) = ( data(:,11).*data(:,15) - data(:,12) - data(:,13) ) ./ ( data(:,15)-2 );
        data=log2(data);
        size(data)
        if strcmp(type{t},'F')
            line='--';
        ...
        legtxt{3*(t-1)+1}=[typstr{t} ' - ' sc];
        hold on
        plot(data(:,1), data(:,7), [line 'rs'], 'MarkerFaceColor', 'r', 'LineWidth', 2, 'MarkerSize', 2)
        legtxt{3*(t-1)+2}=[typstr{t} ' - ' bms];
        plot(data(:,1), data(:,11), [line 'bd'], 'MarkerFaceColor', 'b', 'LineWidth', 2, 'MarkerSize', 2)
        legtxt{3*(t-1)+3}=[typstr{t} ' - ' qms];
    end
    lgd=legend(legtxt{1},legtxt{2},legtxt{3},legtxt{4},legtxt{5},legtxt{6},legtxt{7},legtxt{8},legtxt{9},'Location','Northwest');
    set(lgd, 'fontsize', 14);
...
for v=3:4
    ...
    lgd=legend(legtxt{1},legtxt{2},legtxt{3},'Location','NorthWest');
    set(lgd, 'fontsize', fontsz);
    xlabel('log_2(vector length)','fontsize',fontsz);
    ylabel('log_2(milliseconds)','fontsize',fontsz);
