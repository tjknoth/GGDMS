...
namespace BucketMultiselect_thrust{
  using namespace std;
...
, int numPivots, int sizeOfSample, int totalSmallBuckets, uint min, uint max) {
  ...
  */
  template <typename T>
  T bucketMultiSelect_thrust (T* d_vector, int length, uint * kVals, int numKs, T * output, int blocks
   ...
   */
  template <typename T>
  T bucketMultiselectWrapper_thrust (T * d_vector, int length, uint * kVals_ori, int numKs
    ...
    */
    ...
    bucketMultiSelect_thrust (d_vector, length, kVals, numKs, outputs, blocks, threads, numBuckets, 17);
